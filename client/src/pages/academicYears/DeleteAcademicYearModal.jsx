...
const DeleteAcademicYearModal = ({ open, onClose, academicYear, onSuccess, setError, setLoading }) => {
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      const result = await window.electronAPI.deleteAcademicYear(academicYear.id);
        ...
        <Box className="space-y-4">
          ...
          <Box className="bg-gray-50 p-3 rounded">
            <Typography variant="body2" className="text-gray-700">
              ...
              Period: {new Date(academicYear.start_date).toLocaleDateString()} - {new Date(academicYear.end_date).toLocaleDateString()}<br />
