...
export default function YearManagerDialog({ open, onClose }) {
  ...
  const baseStart = useMemo(() => {
    // Prefer selected year, else the latest (max startDate)
    if (activeYear?.name) {
      const s = parseStartYear(activeYear.name);
      if (s) return s;
    }
    if (Array.isArray(years) && years.length) {
      const sorted = [...years].sort((a, b) => String(b.startDate).localeCompare(String(a.startDate)));
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <FormControl fullWidth>
            ...
            >
              {yearOptions.map((opt) => (
                <MenuItem key={opt.startDate} value={parseInt(opt.name.split("-")[0], 10)}>
                  {opt.name}
                </MenuItem>
              ))}
            </Select>
