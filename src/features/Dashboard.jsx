...
const Dashboard = () => {
  ...
  const [monthStartLabel, setMonthStartLabel] = useState("");
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [studentsRes, staffRes, transportRes, feesRes] = await Promise.all([
        api.getStudents(selectedBranch.id, selectedYear?.id || null).catch(() => []),
        api.getTeachers().catch(() => []),
        api.getTransport().catch(() => []),
        api.getFees(selectedBranch.id, selectedYear?.id || null).catch(() => ({ fees: [] })),
      ]);
      if (!mounted) return;
  ...
  useEffect(() => {
    const now = new Date();
    setHeaderDate(now.toLocaleDateString());
    setMonthStartLabel(
  ...
  }, [students, staff.length, transport.length, fees, getRowDate]);
        ...
        <Grid container spacing={2} alignItems="stretch">
          ...
          <Grid item xs={12} sm={6} md={4}>
            <Card
              ...
              sub={
                monthStartLabel ? `Since ${monthStartLabel}` : "Current month"
              }
              trend={{
