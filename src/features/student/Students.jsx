...
const Students = () => {
  ...
  const columns = useMemo(
    ...
    // Depend only on the action creators so initialization order is safe
  ...
  const fetchClassEntries = useCallback(async (branch_id) => {
    try {
      const data = await api.listClassesByBranch(branch_id);
    ...
    searchTimer.current = setTimeout(async () => {
      try {
        ...
        if (api.searchStudentsByYear) {
          const res = await api.searchStudentsByYear(
            selectedBranch?.id || null,
            q,
            selectedYear?.id || null
          );
          setSearchResults(Array.isArray(res) ? res : []);
        } else if (api.getStudents) {
