...
const StudentFeeDetailsModal = ({ open, onClose, feesRecord }) => {
  ...
  useEffect(() => {
    ...
    const termSummary = feesRecord?.termSummary;
    if (termSummary) {
    ...
    if (!feesRecord?.student_id) {
      setSummary(null);
      setError("Missing student information for this record.");
      setLoading(false);
      return;
    }

    if (!api.getStudentTermSummary) {
      setSummary(null);
      setError("Term summary lookup is unavailable.");
      setLoading(false);
      return;
    }
  ...
  }, [open, feesRecord?.student_id, feesRecord?.academic_year_id, feesRecord?.termSummary]);
