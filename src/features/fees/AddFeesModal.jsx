...
const AddFeesModal = ({
  ...
  useEffect(() => {
    ...
    setData((prev) => {
      if (prev?.payment_date) return prev;
      return { ...(prev || {}), payment_date: today };
    });
  }, [open, setData]);
  ...
  const searchStudents = React.useCallback(
    async (branchId, query) => {
      ...
      setStudentsLoading(true);
      try {
        if (api.searchStudentsByYear) {
          // Use backend search if available
          const data = await api.searchStudentsByYear(
            branchId,
            query,
            selectedYear?.id || null
          );
  ...
  React.useEffect(() => {
    const fetchNextReceipt = async () => {
      try {
        if (!open || !api.getNextReceiptNumber) return;
        const normalized = (data?.payment_type || "cash")
          .toString()
          .trim()
          .toLowerCase();
        const res = await api.getNextReceiptNumber(normalized);
  ...
  React.useEffect(() => {
    const loadSummary = async () => {
      try {
        ...
        if (!api.getStudentTermSummary) return;
        const res = await api.getStudentTermSummary(
          data.student_id,
          selectedYear?.id || null
        );
        if (res?.success) {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const result = await api.addFees(payload);
      if (result?.success) {
