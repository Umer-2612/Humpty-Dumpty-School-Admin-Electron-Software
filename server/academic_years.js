...
function getActiveAcademicYear() {
  return new Promise((resolve, reject) => {
    db.get(
      `SELECT id, name, start_date, end_date, is_active FROM academic_years WHERE is_active = 1 LIMIT 1`,
      [],
      (err, row) => {
        if (err) return reject(err);
        resolve(row || null);
  ...
  return new Promise((resolve, reject) => {
    ...
    db.get(
      ...
      (err, row) => {
        if (err) return reject(err);
        if (row?.is_active) {
          return reject(new Error("Cannot delete the active academic year"));
        }

        // Check if any data is linked to this academic year
        const checkQueries = [
          `SELECT COUNT(*) as count FROM students WHERE academic_year_id = ?`,
          `SELECT COUNT(*) as count FROM fees WHERE academic_year_id = ?`,
        ];
        ...
        checkQueries.forEach((query) => {
          db.get(query, [id], (err, result) => {
            if (err) return reject(err);
            if (result.count > 0) hasLinkedData = true;

            completed++;
            if (completed === checkQueries.length) {
              if (hasLinkedData) {
                return reject(
                  new Error(
                    "Cannot delete academic year with linked data (students/fees)"
                  )
                );
              }
